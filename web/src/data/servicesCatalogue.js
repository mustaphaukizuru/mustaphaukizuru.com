/* ════════════════════════════════════════════════════════════════════════
   ...
   ────────────────────────────────────────────────────────────────────────
   THIS FILE IS THE SOURCE OF TRUTH for the catalogue. docs/catalogue/ is
   generated from it (web/scripts/generate-service-catalog.mjs, checked in CI
   by `npm run catalog:check`), so edit here and regenerate — never the other
   way round. Originally transcribed from the owner's "CONSULTORÍA
   ESTRATÉGICA DE TI" (PDF, 2026-08-24). Structure: 4 categories, 20
   offerings — and nothing should write that number down again. It said 21
   here, 21 in the generator header and 21 in the retired markdown, against a
   catalogue of 20. Read it from CATALOG_STATS. Spanish is the primary voice;
   ...
   Top-level exports (stable API — other modules import these):
     ...
     SERVICES              flat list of every offering (legacy shape:
...
/* ── Pricing basis ───────────────────────────────────────────────────────
 * The header above has cited RATE_USD_PER_HOUR and MXN_PER_USD since this
 * file was written; neither existed, so the two figures lived only in prose
 * and in whichever emitter happened to hardcode them. They are constants now
 * (T2-10), and everything that prints the basis — the PDF catalogue, both
 * generated references, the products seed — reads it from here.
 *
 * 20 MXN/USD is a deliberately stable round conversion, not tracked to live
 * FX: a published price that moved with the exchange rate would make every
 * proposal and every screenshot wrong within a week.
 */
export const RATE_USD_PER_HOUR = 30
 ...
 * generator ran. The generated references are committed and CI re-runs the
 * generator to check them for drift, so anything derived from the clock would
 * make that check fail the day after every commit. Bump this deliberately
 * when the catalogue changes. */
export const CATALOG_LAST_UPDATED = "2026-09-04"
...
export const AUDIENCE_LABELS = {
  // labelEs is not decoration: the Spanish catalogue reference generated by
  // scripts/generate-service-catalog.mjs printed these in English, so a
  // Spanish-language proposal named its own audience in the wrong language.
  SMB: { code: "SMB", label: "SMEs & Businesses", labelEs: "PyMEs y empresas", tone: "azure", priority: "Primary" },
  EDU: { code: "EDU", label: "Schools & Education", labelEs: "Escuelas y educación", tone: "mint", priority: "Secondary" },
  IND: { code: "IND", label: "Individuals & Pros", labelEs: "Personas y profesionales", tone: "terracotta", priority: "Inbound" },
}
...
/* ── The categories and their offerings — counts live in CATALOG_STATS ───
