...
const FILE_TYPE_MAP = {
  ...
  jpeg: { icon: ImageIcon, label: "JPG", tone: "cyan" },
...
export function getFileTypeMeta(fileType) {
  ...
  // NOTE: Aliases are checked AFTER normalizeFileType strips x- / vnd. / ms- prefixes,
  // so the keys here are the post-strip forms (e.g. "msvideo" -> "video" after
  // ms- strip, so we use "video" as the key, but only when input was a video MIME).
  // Where the post-strip is ambiguous, we add multiple alias forms.
  ...
  // Archive MIME variants
  if (ext === "zip-compressed" || ext === "x-zip-compressed") return FILE_TYPE_MAP.zip
  if (ext === "rar-compressed" || ext === "x-rar-compressed") return FILE_TYPE_MAP.rar
  if (ext === "7z-compressed" || ext === "x-7z-compressed") return FILE_TYPE_MAP["7z"]
  if (ext === "gzip" || ext === "tgz") return FILE_TYPE_MAP.gz
  if (ext === "tar") return FILE_TYPE_MAP.tar

  // Image MIME variants
  if (ext === "jpeg") return FILE_TYPE_MAP.jpg
  if (ext === "svg") return FILE_TYPE_MAP.svg

  // Video MIME variants — note: post-normalization "msvideo" -> "video" via ms-
  // strip, but normalizeFileType only strips ms- when it's the prefix of the
  // ext. For "x-msvideo" -> "msvideo", we still want msvideo here:
  if (ext === "msvideo") return FILE_TYPE_MAP.avi
  if (ext === "quicktime") return FILE_TYPE_MAP.mov
  if (ext === "matroska") return FILE_TYPE_MAP.mkv

  // Audio MIME variants
  if (ext === "mpeg") return FILE_TYPE_MAP.mp3
  if (ext === "flac") return FILE_TYPE_MAP.flac
  if (ext === "wav") return FILE_TYPE_MAP.wav

  // Text MIME variants
  if (ext === "plain") return FILE_TYPE_MAP.txt
  if (ext === "markdown") return FILE_TYPE_MAP.md

  // Unknown extension — generic file record.
  // Strip non-alphanumeric chars from label and cap at 4 chars to avoid weird
  // truncations like "ZIP-" or "X-ZI" — gives clean labels for unknown types.
  const cleanLabel = ext.replace(/[^a-z0-9]/gi, "").slice(0, 4).toUpperCase()
  return { icon: FileIcon, label: cleanLabel || "FILE", tone: "default" }
}
