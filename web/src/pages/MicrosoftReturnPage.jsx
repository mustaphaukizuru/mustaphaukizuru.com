...
/**
 * MicrosoftReturnPage · OAuth redirect-flow landing
 * Identical lifecycle to GoogleReturnPage — the backend redirects here with
 * token + user in the URL fragment after /api/auth/microsoft/callback.
 */
export default function MicrosoftReturnPage() {
  ...
  useEffect(() => {
    function consume() {
      try {
        ...
        setError(true)
        setTimeout(() => navigate("/login?microsoft=exchange_failed", { replace: true }), 600)
      }
    }
    consume()
