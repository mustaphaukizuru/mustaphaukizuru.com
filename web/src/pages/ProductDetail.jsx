    ...
    <>
      ...
      <div className="bg-mist overflow-x-hidden w-full max-w-[100vw]">
        <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
          <nav className="mb-6 flex flex-wrap items-center gap-2 text-meta text-charcoal-80/65">
            ...
            <span className="text-charcoal-80/65">{product.category || t("misc.product")}</span>
            <ChevronRight className="h-3.5 w-3.5" />
          ...
          <div className="grid gap-10 lg:grid-cols-[1fr_400px] xl:grid-cols-[1fr_440px]">
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="rounded-xl border border-charcoal-80/10 bg-white shadow-[var(--shadow-e3)]">
                  ...
                  {tabs.map((tab) => (
                    <button
                      ...
                      className={`inline-flex shrink-0 items-center justify-center gap-1.5 whitespace-nowrap px-4 py-3.5 text-meta font-semibold transition-all sm:flex-1 ${
                        activeTab === tab.id ? "border-b-2 border-violet text-violet" : "text-charcoal-80/65 hover:text-violet"
                      }`}
                    >
                      <span>{tab.label}</span>
                      {tab.count != null && (
                        <span
                          className={`rounded-full px-1.5 py-0.5 font-mono text-micro font-bold tabular-nums ${
                            activeTab === tab.id ? "bg-violet-pale text-violet" : "bg-charcoal-80/8 text-charcoal-80/65"
                          }`}
                        >
                          {tab.count}
                        </span>
                    ...
                    <div className="space-y-6">
                      ...
                      {fileCount > 0 && (
                        <div>
                          <div className="mb-3 flex items-baseline justify-between">
                            ...
                            <span className="font-mono text-micro text-charcoal-80/65 tabular-nums">
                      ...
                      {highlights.length === 0 && fileCount === 0 && (
                        <div className="flex flex-col items-center justify-center gap-2 rounded-xl border border-dashed border-charcoal-80/15 bg-mist/40 px-6 py-10 text-center">
                          ...
                          <p className="text-meta font-semibold text-charcoal-80/65">{t("detail.contentsComing")}</p>
                        </div>
                      )}
                    </div>
                  )}
