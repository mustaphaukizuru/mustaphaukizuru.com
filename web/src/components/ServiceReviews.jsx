/* ════════════════════════════════════════════════════════════════════════
   ServiceReviews.jsx · public review surface for the service detail page
   ────────────────────────────────────────────────────────────────────────
   Mirror of ProductReviews.jsx — same UX, same atoms — but pointed at the
   service-side endpoints. Mounted on ServiceDetailPage.

   Every behavioural choice intentionally matches ProductReviews so the two
   feel like one product surface. If you tweak one, tweak both.
   ════════════════════════════════════════════════════════════════════════ */
...
/* ─── atoms (identical to ProductReviews) ─────────────────────────────── */
  ...
  return (
    <button
      ...
      className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-micro font-semibold transition ${
        voted ? "bg-violet text-white" : "bg-violet-pale/60 text-violet hover:bg-violet-pale"
      } ${disabled && !voted ? "opacity-60" : ""}`}
    >
      <ThumbsUp className={`h-3 w-3 ${voted ? "fill-white" : ""}`} />
      <span>Helpful</span>
      {count > 0 && (
        <span className={`font-mono tabular-nums ${voted ? "text-white" : "text-violet"}`}>· {count}</span>
      )}
    </button>
  )
}
...
export default function ServiceReviews({ slug, serviceTitle }) {
  ...
  const loadReviews = useCallback(async () => {
    try {
      setLoading(true)
      const data = await fetchServiceReviews(slug)
  ...
  async function handleSubmit(e) {
    ...
    try {
      setSubmitting(true)
      const result = await submitServiceReview(slug, { rating: formRating, reviewText: formText.trim() })
      setFormSuccess(
        result?.message
  ...
  async function handleDelete(reviewId) {
    ...
    try {
      await deleteServiceReview(slug, reviewId)
      await loadReviews()
    } catch { /* silent */ }
  }
    ...
    <div className="space-y-6">
      ...
      {alreadyReviewed && !formSuccess && (
        <div className="rounded-xl border border-violet/10 bg-violet-ghost px-5 py-3 text-sm text-charcoal-80/70">
          ...
          {t("reviews.alreadyReviewed")} {serviceTitle ? <em>{serviceTitle}</em> : "this service"}.
        </div>
      )}
                      ...
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        {review.isVerifiedPurchase && (
                          <span className="flex items-center gap-1 rounded-full bg-mint-600/10 px-2 py-0.5 text-micro font-bold text-mint-600">
                            <CheckCircle2 className="h-3 w-3" /> {t("reviews.verifiedClient")}
                          </span>
                        )}
                        {review.featured && (
